...
export default function markPreviousAttendance(db) {
  const d = new Date();
  ...
  router.post("/", async (req, res) => {
    if (req.isAuthenticated()) {
      ...
      if(classRecord.rows.length>0){
        ...
        res.render("./attendance/markPreviousAttendance.ejs", {
          userData: req.user,
  ...
  router.post("/save", async (req, res) => {
    if (req.isAuthenticated()) {
          const { selectdate, attendance, psfid } = req.body;

          const checkAttendance = await db.query(
            `select * from attendance where psf_id=$1 AND adate=$2`,[psfid, selectdate]
          );
          ...
          if (checkAttendance.rows.length > 0) {
                ...
                for (const enrollmentid in attendance) {
                          ...
                          values.push(`(${psfid},${enrollmentid},'${selectdate}','${status}','${pgdate}',false)`
                  );
                }
